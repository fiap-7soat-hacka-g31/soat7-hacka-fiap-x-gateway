...
resource "aws_apigatewayv2_integration" "fiap_burger_identity_integration" {
  ...
  integration_uri  = data.terraform_remote_state.gateway.outputs.fiap_burger_identity_invoke_arn
}
...
resource "aws_lambda_permission" "api_gtw" {
  ...
  function_name = data.terraform_remote_state.gateway.outputs.fiap_burger_identity_function_name
